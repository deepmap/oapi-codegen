...
type RoundTripMiddlewares struct {
	ExampleGet RoundTripMiddleware
}
...
func setupOperationDoers(c *Client, rtMiddlewares RoundTripMiddlewares) *operationDoFunctions {
	...
	// ExampleGet
	if rtMiddlewares.ExampleGet != nil {
		mw := joinMiddleware(sharedMiddleware, rtMiddlewares.ExampleGet)
		operationDoers.ExampleGet = mw.DoFn(c.Client.Do)
	} else {
		operationDoers.ExampleGet = sharedMiddleware.DoFn(c.Client.Do)
	}
...
func (c *Client) ExampleGet(ctx context.Context) (*http.Response, error) {
	...
	return c.operationDoers.ExampleGet(req)
}
