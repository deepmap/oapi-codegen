...
- `x-dynamodb`: specifies if a dynamodbav gets added to the field tag. There is two options either
  full or omitempty. If omitempty the omitempty attribute will be added on to the end of the tag
  with the same rules as currently exist for the json tag.
  ...
  schemas:
    Pet:
      ...
      properties:
        ...
        age:
          type: string
          x-dynamodb: omitempty
        dob:
          type: string
          x-dynamodb: omitempty
  ```

  In the example above the following go struct will be created.
